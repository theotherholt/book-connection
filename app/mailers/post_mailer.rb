class PostMailer < ActionMailer::Base # :nodoc:
  def sold_notification(post)
    ...
    @subject   += "Your Copy of \"#{ActionController::Base.helpers.truncate(post.book.title, :length => 50)}\" Has Sold"
  end
  
  def purchased_notification(post)
    setup_email(post)
    @recipients = post.buyer.email
    @subject   += "You've Purchased \"#{ActionController::Base.helpers.truncate(post.book.title, :length => 50)}\""
  end
